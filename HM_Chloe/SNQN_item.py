...
def parse_args():
    ...
    parser.add_argument("--data", nargs="?", default="data2", help="data directory")
...
class QNetwork:
    def __init__(
        ...
        #####Modify
        self.feature_vectors = item_feature_data
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.output1 = tf.compat.v1.layers.dense(
                self.states_hidden, self.item_num, activation=None, name="all-q-values"
            )  # all q-values #[batch_size, item_num]
            ###############modify here################
            self.output_phi = tf.compat.v1.layers.dense(
                self.states_hidden, self.item_num, activation=None, name="ce-logits"
            )  # all ce logits # [batch_size, item_num] # phi-score

            self.feature_embeddings = tf.compat.v1.layers.dense(
                self.feature_vectors,  # [item_num, feature_dim]]
                self.states_hidden.shape[1] + 1,  # hidden state dim + bias
                activation=None,
                name="feature-embeddings",
            )  # feature embeddigns #[item_num, hidden state dim + 1]

            # concat (matrix multiplication of feature embeddings and hidden state)  and bias
            self.output_phi_prime = tf.matmul(
                self.states_hidden,
                self.feature_embeddings[:, :-1],
                transpose_b=True,
            ) + tf.transpose(self.feature_embeddings[:, -1])
            # [batch_size, item_num] # phi' score
            self.output2 = (
                1 - self.alpha
            ) * self.output_phi + self.alpha * self.output_phi_prime
            ###############modify here################
...
if __name__ == "__main__":
    ...
    data_statis = pd.read_pickle(
        os.path.join(data_directory, "data_statis.pkl")
    ...
    item_category = pd.read_pickle(os.path.join(data_directory, "item_category.pkl"))
    item_category = item_category.values.astype("float32")
    # item_category: [number_items x number_categories]
    ...
    QN_1 = QNetwork(
        ...
        pretrain=False,
        item_feature_data=item_category,
    )
    QN_2 = QNetwork(
        name="QN_2",
        hidden_size=args.hidden_factor,
    ...
    replay_buffer = pd.read_pickle(os.path.join(data_directory, "replay_buffer.pkl"))
