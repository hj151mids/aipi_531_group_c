...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, name='DQNetwork'):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.len_state = tf.compat.v1.placeholder(tf.int32, [None])  # the length of valid positions, because short sesssions need to be padded
            ...
            self.features = tf.compat.v1.placeholder(tf.int32, [None])
            ...
            self.weights = tf.compat.v1.layers.dense(self.features, , activation_fn=None, scope=" ")
            # TRFL way
...
if __name__ == '__main__':
    ...
    item_features = pd.read_csv('data/features.csv')
    features = item_features.iloc[:,1:].values
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for i in range(args.epoch):
            for j in range(num_batches):
                ...
                loss, _, logits = sess.run([mainQN.loss, mainQN.opt, mainQN.output2],
                                   feed_dict={mainQN.inputs: state,
                                              mainQN.features: features
                                              mainQN.len_state: len_state,
